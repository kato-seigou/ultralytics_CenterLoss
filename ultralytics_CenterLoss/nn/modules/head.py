...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        # 追加
        self.cls_feat = nn.ModuleList( # あ
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3)) for x in ch) # あ
        self.cls_pred = nn.ModuleList(nn.Conv2d(c3, self.nc, 1) for _ in ch) # あ
        
        # 互換性確保あ
        self.cv3 = self.cls_pred
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[List[torch.Tensor], Tuple]:
        ...
        self.saved_cls_feats = []          # どちらのモードでも毎回リセット

        for i in range(self.nl):
            feat_i = self.cls_feat[i](x[i])
            cls_logits = self.cls_pred[i](feat_i)

            if self.training:              # Center Loss 用に保存
                # (B,C,H,W) → (B*H*W, C) に揃えたいので flatten しておくと後段が楽
                # self.saved_cls_feats.append(feat_i)
                # self.saved_cls_feats.append(feat_i.detach())
                self.saved_cls_feats.append(feat_i) # これでCenterLossの逆伝播が可能
                
            x[i] = torch.cat((self.cv2[i](x[i]), cls_logits), 1)

        if self.training:
            return x                       # list[3]   (box+cls logit を積んだ fmap)

        y = self._inference(x)             # self.saved_cls_feats は推論でも保持可
    ...
    def decode_bboxes(self, bboxes: torch.Tensor, anchors: torch.Tensor, xywh: bool = True) -> torch.Tensor:
        """
        Ultralytics 標準の bbox デコード
        bboxes  : (B, A, 4)   – self.dfl() で (reg_max*4→4) に変換済み
        anchors : (1, A, 2)   – make_anchors() で生成
        """
    ...
    def bias_init(self):
        ...
        if self.end2end:
            for a, b, s in zip(m.one2one_cv2, m.one2one_cv3, m.stride):  # from
                ...
                b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img)"""
        # 変更箇所あ
        for box_conv, cls_conv, stride in zip(self.cv2, self.cls_pred, self.stride):
            box_conv[-1].bias.data[:] = 1.0                                    # box/obj
            cls_conv.bias.data[: self.nc] = math.log(5 / self.nc / (640 / stride) ** 2)

        # ────────────────────────────────
        # End-to-End (O2O) ヘッドを使う場合のみ
        if self.end2end:
            for box_conv, cls_conv, stride in zip(self.one2one_cv2, self.cls_pred, self.stride):
        ...
        if hasattr(self, "saved_cls_feats"):
            self.saved_cls_feats.clear()
        # 変更箇所終わりあ
